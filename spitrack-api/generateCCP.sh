...
sleep 5
  ...
  NO_OF_ORDERERS=1
  echo ${NO_OF_ORDERERS}
  CPWD="/home/paperspace/SpiTrack"
  SourcePATH="/home/paperspace/SpiTrack/spitrack-api"
  # CPWD="/usr/CLIF"
  ...
  declare -A CHANNELS
  T_ORGS=3
  orgDetails[0,0]="Importer"
  orgDetails[1,0]="Exporter"
  orgDetails[2,0]="Supplier"
  orgDetails[0,2]="Importer"
  orgDetails[1,2]="Exporter"
  orgDetails[2,2]="Supplier"
  orgDetails[0,1]=1
  orgDetails[1,1]=1
  orgDetails[2,1]=1
  CCC_NAME="spitrackcc"
  CHANNELS[0,0]="spitrackchannel"
IP=$(hostname -I|awk {'print $1'})
 ...
 MOPath="${CPWD}/"
  cd $MOPath
  ordererpath="${MOPath}crypto-config/ordererOrganizations/spitrack.com/orderers"



  for count in `seq 0 $Mxca`  
  do 
  Orderer_Ca[${count}]="$(awk '{printf "%s\\n", $0}' ${ordererpath}/orderer.spitrack.com/tls/ca.crt)"
...
do 
 MOPATH="${CPWD}/"
 Org_Connection[${orgDetails[${countcon},0]}cacert]="$(awk '{printf "%s\\n", $0}' ${MOPATH}crypto-config/peerOrganizations/${orgDetails[${countcon},0]}.spitrack.com/ca/ca.${orgDetails[${countcon},0]}.spitrack.com-cert.pem)"
 Org_Connection[${orgDetails[${countcon},0]}peercert]="$(awk '{printf "%s\\n", $0}' ${MOPATH}crypto-config/peerOrganizations/${orgDetails[${countcon},0]}.spitrack.com/peers/peer0.${orgDetails[${countcon},0]}.spitrack.com/tls/ca.crt)"
...
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json
            "peer${peercount}.${orgDetails[${peer},0]}.spitrack.com": {
            "endorsingPeer": true,
            "chaincodeQuery": true,
            "eventSource": true
                  }
...
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json
            "peer${peercount1}.${orgDetails[${peer},0]}.spitrack.com": {
            "endorsingPeer": true,
            "chaincodeQuery": true,
            "eventSource": true
          },
...
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json          
            "peer${peercountorder}.${orgDetails[${countorder},0]}.spitrack.com"
        ],
EOF
          else
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json          
...
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json 
        "certificateAuthorities": [
          "ca.${orgDetails[${countorder},0]}.spitrack.com"
        ]
EOF
...
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json
      ...
      "grpcOptions": {
        "ssl-target-name-override": "peer${peercount}.${orgDetails[${peer},0]}.spitrack.com",
        "hostnameOverride": "peer${peercount}.${orgDetails[${peer},0]}.spitrack.com"
      },
      "tlsCACerts": {
...
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json
    "ca.${orgDetails[${countco},0]}.spitrack.com": {
      "url": "http://${IP}:${ca_port}",
      "caName": "ca.${orgDetails[${countco},0]}.spitrack.com",
      "httpOptions": {
        "verify": false
      },
      "tlsCACerts": {
